...
class PlaylistDownloader:
    ...
    def setup_ui(self):
        ...
        ttk.Button(scan_frame, text="Escanear URL", command=self.scan_playlist).pack(side="left")
        ...
        # Substituir o botão "Baixar Tudo" por "Atualizar Lista"
        self.refresh_btn = ttk.Button(btn_frame, text="Atualizar Lista", command=self.scan_playlist)
        self.refresh_btn.pack(side="left", padx=(0, 5))
        
        # Mantemos os outros botões para funcionalidade completa
        self.download_btn = ttk.Button(btn_frame, text="Baixar Tudo", command=self.start_download)
        self.download_btn.pack(side="left", padx=(0, 5))
        
        self.retry_btn = ttk.Button(btn_frame, text="Tentar Baixar Falhas", 
                                  command=self.retry_failed_downloads, state="disabled")
        self.retry_btn.pack(side="left", padx=(0, 5))
        
        self.cancel_btn = ttk.Button(btn_frame, text="Cancelar", command=self.cancel_download, state="disabled")
            ...
            if "list" in query_params:
                ...
                if is_mix and "v" in query_params:
                    ...
                    self.root.after(0, lambda: messagebox.showinfo(
                        ...
                        "copie a URL de cada vídeo individualmente."
                    ))
                    return fixed_url
    ...
    def scan_playlist(self):
        ...
        def do_scan():
            try:
                ...
                is_playlist = self.is_playlist_url(original_url)
                
                # Determinar plataforma automaticamente
                detected_platform = self.detect_platform(original_url)
                self.add_to_log(f"Plataforma detectada: {detected_platform}")
                
                # Tratamento especial para URLs
                url = self.fix_youtube_music_url(original_url)
                is_modified_url = url != original_url
                ...
                self.count_label.config(text=f"Arquivos: {valid_entries}")
                
                if valid_entries > 0:
                    # Mensagem mais clara para download individual
                    self.status_label.config(text="Clique em '⬇️ Baixar' ao lado do item para baixar individualmente.")
                    self.add_to_log("Lista pronta. Para baixar um item, clique no botão '⬇️ Baixar' ao lado dele.")
